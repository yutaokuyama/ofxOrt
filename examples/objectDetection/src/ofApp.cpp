...
//--------------------------------------------------------------
void ofApp::update() { scale = (ofGetWidth() / 1.3) / original.getWidth(); }
...
void ofApp::inference(ofFloatImage &content) {
  ...
  objectDetectionWithModelOutput(output_tensor.getTexData(), bboxes, scores,
                                 classIndices, confidenceThresh);
...
void ofApp::exit() { delete ort; }

void ofApp::objectDetectionWithModelOutput(
    const std::vector<float> &inferenceOutput,
    std::vector<std::array<float, 4>> &bboxes, std::vector<float> &scores,
    std::vector<uint64_t> &classIndices, const float confidenceThresh) {

  std::vector<float> outputData{inferenceOutput.begin(),
                                inferenceOutput.begin() + NUM_BOXES};
  ...
  for (uint64_t i = 0; i < FEATURE_MAP_SIZE; ++i) {
    for (uint64_t j = 0; j < NUM_ANCHORS; ++j) {
      for (uint64_t k = 0; k < m_numClasses; ++k) {
        tmpScores[k] =
            outputData[i + FEATURE_MAP_SIZE * ((m_numClasses + 5) * j + k + 5)];
      }

      ofxOrtUtils::softmax(tmpScores, m_numClasses);
      uint64_t maxIdx = std::distance(
          tmpScores, std::max_element(tmpScores, tmpScores + m_numClasses));
      float probability = tmpScores[maxIdx];

      if (ofxOrtUtils::sigmoid(
              outputData[i + FEATURE_MAP_SIZE * ((m_numClasses + 5) * j + 4)]) *
              probability >=
          confidenceThresh) {
        float xcenter =
            (ofxOrtUtils::sigmoid(
                 outputData[i + FEATURE_MAP_SIZE * (m_numClasses + 5 * j)]) +
             i % 13) *
            32.0;
        float ycenter = (ofxOrtUtils::sigmoid(
                             outputData[i + FEATURE_MAP_SIZE *
                                                ((m_numClasses + 5) * j + 1)]) +
                         i % 13) *
                        32.0;

        float width = expf(outputData[i + FEATURE_MAP_SIZE *
                                              ((m_numClasses + 5) * j + 2)]) *
                      ANCHORS[2 * j] * 32.0;
        float height = expf(outputData[i + FEATURE_MAP_SIZE *
                                               ((m_numClasses + 5) * j + 3)]) *
                       ANCHORS[2 * j + 1] * 32.0;
...
void ofApp::drawBoundingBox(std::array<float, 4> &bbox, std::string &className,
                            float score) {

  ofPushMatrix();
  ofScale((original.getWidth() / fbo.getWidth()) * scale,
          (original.getHeight() / fbo.getHeight()) * scale);
  ofTranslate(bbox[0], bbox[1]);
  ofFill();
  ofSetColor(255);
  ofDrawRectangle(0.0, 0.0, abs(bbox[0] - bbox[2]), 14.0);
  ofSetColor(0);
  ofDrawBitmapString(ofToString(className) + " " + ofToString(score), 0.0,
                     12.0);
  ofSetColor(255);
  ofNoFill();
