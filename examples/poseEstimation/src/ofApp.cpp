...
void ofApp::inference(ofFloatImage &content) {
  ...
  ofxOrtUtils::splitImageDataArray(output_tensor.getTexData(), dstArray,
                                   numChannel, width, height);
  std::vector<ofFloatImage> heatmaps =
      ofxOrtUtils::buildImagesFromData(dstArray, width, height);
